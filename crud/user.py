...
async def get_user_by_token(db: AsyncSession, token: str):
    ...
    result = await db.execute(query)
    return result.scalar_one_or_none()###一定要返回，用于下一步判断
...
async def change_password(db: AsyncSession, user: User, old_password: str, new_password: str):###如果传递的参数中直接存在User可以直接进行修改然后提交事务
